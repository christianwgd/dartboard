...
class MatchSummaryView(LoginRequiredMixin, DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        p1_turns = turns.filter(player_id=self.object.player1)
        p2_turns = turns.filter(player_id=self.object.player2)
        if p1_turns:
            ctx["player1_average"] = round(sum(turn.score for turn in p1_turns) / len(p1_turns), 1)
        else:
            ctx["player1_average"] = 0
        if p2_turns:
            ctx["player2_average"] = round(sum(turn.score for turn in p2_turns) / len(p2_turns), 1)
        else:
